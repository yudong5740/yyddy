 ...
 <div class="contains_content_Otitle">
            ...
            <li v-for="(item,index) in bbostlist" >{{item}}<span @click="delates(index)" style="color:red">删除</span></li>
             ...
             <template v-if="isDelates">
                 <template v-if="delateing">
                    ...
                    <span>正在处理文件（{{delatedNum}}/{{key}}）<template v-if="!isFree">，已消耗{{delated_blance}}额度</template></span>
                 </template>
                 <template v-else> 
             ...
             <template v-else>
                <template v-if="allComplete">  <!--allComplete-->
                  ...
                  {{key}}个文件等待处理<template v-if="!isFree">，需要消耗{{consume_balance}}额度</template>
                  </span>
                </template>
                <template v-else-if="isUploading">  <!--isUploading-->
                  <i class="el-icon-loading"></i>
                  <span>正在上传（{{key}}/{{sum}}）<template v-if="!isFree">，需要消耗{{consume_balance}}额度</template></span>
                </template>
             </template> 
        </div>
             ...
             <!--<li v-for="(item,index) in bbostlist">-->
                ...
                <template v-else>
                    ...
                    <template v-else> 
                      ...
                      <template v-else>
                          ...
                          <template v-else-if="item.singleComplete||item.noServe">
                            <!-- <span>服务异常，请联系客服或重试{{item.noServe}},{{item.status}},{{item.singleComplete}},</span> -->
                                  <span>{{item.noServeMessage}},{{item.status}},{{item.singleComplete}},</span>
                          </template>
                      </template>
              ...
              <div class="loading_states">
               <template v-if='index_u_status'>    
                  ...
                  <template v-else-if="item.price||item.singleComplete">
                    <span  @click="delates(index,'删除')" class="delates" >删除</span>
                  </template>
                  <template v-else-if="item.noServe">
                    <span  @click="delates(index,'网络异常删除')" class="delates" >删除</span>
                  </template>
                  <template v-else>
    ...
    background: blue;">1 额度</span>
        ...
        <div  v-show="isOnFont" >
          ...
          <div class="Water_font"  :class="{changesOver:!changesOver}">
            ...
            <template v-if="changesOver">
              <input  type="text" class="inputText actices" :class="{acticess:inputTextSize}"  v-model="fontSize" @mouseenter="mouseEnters('inputTextSize')" @mouseleave="mouseLeaves('inputTextSize')"  >
              <!-- @blur="fontSizeBlur" -->
            </template>
            <template v-else>
            ...
            <div class="bgColorS">
              ...
              <template v-else> 
                ...
                <div  :style="{background:colorListSure.colorMessage}">
...
export default {
  ...
  data() {
    return {
      ...
      chose_img_btn_status: "loading", //水印内容的选择图片之后的状态
      chose_img_btn: {
        //水印内容的选择图片之后的状态类型
      ...
      chose_imgColor: false, //鼠标移入是否高亮
      textareaColor: false, //鼠标移入是否高亮
      inputTextSize: false, //鼠标移入是否高亮
      color: "#ff0000",
      // 问下后端，上传水印图片是否需要参数
      datas: {
        file: "",
  ...
  watch: {
    ...
    textContent(val) {
      //文字内容
      if (val && this.allComplete && this.key > 0) {
        this.isBtnStartChanges = true;
      } else {
        this.isBtnStartChanges = false;
      }
    ...
    fileLists(val) {
      //选择图片
      if (this.key > 0 && this.allComplete && this.fileLists.length > 0) {
        this.isBtnStartChanges = true;
      } else {
        this.isBtnStartChanges = false;
      }
    ...
    fontSize() {
      var that = this;
      clearInterval(this.fontSizeTime);
      this.fontSizeTime = setTimeout(function() {
        if (that.fontSize > 128 || that.fontSize < 6 || that.fontSize == 6) {
          that.fontSize = 6;
        } else if (!Number(that.fontSize)) {
          that.fontSize = 6;
        } else {
    ...
    bbostlist(val) {
      ...
      var noServeNum = val.filter(item => item.noServe);
      var noWebNum = val.filter(item => item.noWeb);
      if (
        this.allComplete &&
        this.key > 0 &&
        (nums.length == this.key ||
          noServeNum.length + nums.length + noWebNum.length == this.sum)
      ) {
        if (
          (this.textContent && this.isOnFont) ||
          (this.fileLists.length > 0 && !this.isOnFont)
  ...
  mounted() {
    ...
    this.$leoBus.$on("allCompleteUpload", () => {
      ...
      var priceNum = that.bbostlist.filter(item => item.price);
      if (priceNum.length == that.key) {
  ...
  methods: {
    ...
    mouseLeaves(name) {
      this[name] = false;
      // if(name=="inputTextSize"){
      //   this.fontSize=0
      // //      if (this.fontSize > 128 || this.fontSize <6 || this.fontSize == 6) {
      // //   this.fontSize = 6;
      // // }  {
    ...
    handleError(err, file, fileListss) {
      this.chose_img_btn_status = "error";
    ...
    btnTask(numlists) {
      ...
      var listDate = {
        ...
        opcity: Number(this.opcityListSure.value), //透明度   this.opcityListSure.value
        fontName: this.fontListSure.value, //字体名称
        fontSize: Number(this.fontSize), //字体大小
        color: this.colorListSure.value, //文字颜色
        startPage: 0, //默认参数
        endPage: -1, //默认参数
        bOnTop: this.isOnPlace //放置层
      };
      var listDate_img = {
        opcity: Number(this.opcityListSure.value), //透明度that.opcityMessage
        rotate: Number(this.roteListSure.value), //旋转角度
        startPage: 0, //默认参数
        endPage: -1, //默认参数
        bOnTop: this.isOnPlace, //放置层
