<template>
<!-- v-show="false" -->
  <div class="uploader-file" :status="status"  v-show="false" >
    <slot
      :file="file"
      :list="list"
      :status="status"
      ...
      <div class="uploader-file-info">
        <div class="uploader-file-name"><i class="uploader-file-icon" :icon="fileCategory" v-if='false'></i>{{file.name}}{{progressStyle.progress}}   {{statusText}} </div>
        <template v-if="progressStyle.progress !='100%'">
    ...
    background: #ff8e68;
    height: 6px;
    border-radius: 3px;" :style="{width:progressStyle.progress}"></div>

    </div>
     <span style="font-size: 12px;
    margin-left: 18px;">正在上传</span>
     </div>
             <!-- <div class="uploader-file-size">{{formatedSize}}</div>
             <div class="uploader-file-limit">额度  {{statusText}}</div> -->
        </template>
        <template v-else>
             <template  v-if='statusText=="成功了"'>
                <div class="uploader-file-size">{{formatedSize}}</div>
                <div class="uploader-file-limit">额度{{file.price}}  </div>
             </template>
             <template v-else>
                <div class="uploader-file-err">最大支持100M文件处理，您可尝试压缩文档后重试</div>
             </template>
        </template>
        ...
        <div class="uploader-file-actions">
          ...
          <template v-if='statusText=="成功了"||statusText=="出错了"'>
            <span class="uploader-file-remove" @click="remove">删除</span>
          </template>
          <template v-else>
            <span class="uploader-file-remove"  @click="remove">取消上传</span>
          </template>
...
export default {
  ...
  computed: {
    ...
    statusText() { 
      ...
      Object.assign(this.file, { status: this.status });
